...
func getDesiredState() (map[string]*model.App, error) {
	sql := "select a.app_name name, a.app_memory memory, a.app_instance instance, a.app_image image, a.app_status status, a.app_port port, a.app_mount mount, a.app_cpushares, a.app_cpuset from ysy_app a where a.app_status = 0 and a.app_image <> ''"
	...
	for rows.Next() {
		var app model.App
		err = rows.Scan(&app.Name, &app.Memory, &app.InstanceCnt, &app.Image, &app.Status, &app.Port, &app.Mount, &app.CPUShares, &app.CPUSet)
		if err != nil {
